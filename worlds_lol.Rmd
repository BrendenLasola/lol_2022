...
#patch at 12.18 because data was included to WCS that was not apart of the tournament
worlds <- lol_data %>% 
  filter(league =='WCS') %>%
  filter(patch == '12.18')
```
...
num_bot <- function(name) {
  ...
  bot_graph <- ggplot(num_played, aes(x = reorder(champion, -total_count), y = total_count)) +
  geom_bar(stat="identity", color='skyblue',fill='steelblue') + xlab('Champion') + ylab('# Picked')
...
num_mid <- function(name) {
  ...
  mid_graph <- ggplot(num_played, aes(x = reorder(champion, -total_count), y = total_count)) +
  geom_bar(stat="identity", color='skyblue',fill='steelblue') + xlab('Champion') + ylab('# Picked')
...
num_sup <- function(name) {
  ...
  sup_graph <- ggplot(num_played, aes(x = reorder(champion, -total_count), y = total_count)) +
  geom_bar(stat="identity", color='skyblue',fill='steelblue') + xlab('Champion') + ylab('# Picked')
...
num_jung <- function(name) {
  ...
  jung_graph <- ggplot(num_played, aes(x = reorder(champion, -total_count), y = total_count)) +
  geom_bar(stat="identity", color='skyblue',fill='steelblue') + xlab('Champion') + ylab('# Picked')
...
num_top <- function(name) {
  ...
  top_graph <- ggplot(num_played, aes(x = reorder(champion, -total_count), y = total_count)) +
  geom_bar(stat="identity", color='skyblue',fill='steelblue') + xlab('Champion') + ylab('# Picked')
